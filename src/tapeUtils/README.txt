...
"7punch". Multics uses the 7-punch format to write binary records to card
images. The images contain sequence numbers and checksums to verify the
deck integrity.

  The 7-punch data generated by Multics is an 81 column word format, which
is an integral number of 36 bit words. The last column (12 bits) is ignored.
That format maps to 121.5 bytes, so for host file representation, the last 12 bits (the 81st column) are elided, leaving 120 byte card images.
